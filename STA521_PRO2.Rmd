...
cbind(floor(sort(x)/38),sort(x))

sort(unique(data$block_y))
```
...
log_fit <- train(as_factor(expert_labels) ~ NDAI + SD + CORR + DF, data = train_data, 
                 method = "glm", 
                 trControl = fitControl,
                 family = "binomial"
                 )
...
```{r}
#cv master for ridge regression 
CVmaster(classifier='ridge', features=features, labels=labels,nfold=5)
```
...
```{r}
#ROC for logistic
test_logit_pred = predict(log_fit,newdata=test_data)
logit.threshold = 0.5
test_logit_roc = roc(factor(test_data$expert_labels,ordered=TRUE),
                     factor(test_logit_pred,ordered = TRUE),plot=TRUE,print.auc=TRUE,print.thres = logit.threshold,main='ROC Curve for Logistic Regression')
coords(test_logit_roc, "best", ret = "threshold")
test_logit_roc$thresholds[which.max(test_logit_roc$specificities+test_logit_roc$sensitivities)]
```

Confusion matrix for logistics regression is:
```{r}
confusionMatrix(as.factor(test_data$expert_labels),as.factor(test_logit_pred))
```
