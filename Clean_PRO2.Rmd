...
cv_knn_y = c()
for(k in ks){
  error = CVmaster('knn',features=featuresy, labels=labelsy,k = k,nfold = 5)
  cv_knn_y =c(cv_knn_y,error)
}
```

Plot for knn error
```{r}
knndt = as.data.frame(cbind(ks,cv_knn_y))
theme_update(plot.title = element_text(hjust = 0.5))
ggplot(data=knndt,aes(x=ks,y=cv_knn_y))+
  geom_line()+
  ggtitle('CV Test Error Versus Neighbour Number Under Split Along Y')+
  xlab('Neighbour Size')+
  ylab('Test Error Across All Folds')+
  labs(captions='Fig 3.2')
```
